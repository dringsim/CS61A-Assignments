...
def calc_eval(exp):
    ...
    elif isinstance(exp, (int, float)) or isinstance(exp, bool):   # Numbers and booleans
        return exp
    elif isinstance(exp, str): # CHANGE THIS CONDITION FOR Q4 where are variables stored?
...
def floor_div(args):
    ...
    20
    """
    result = calc_eval(args.first) // calc_eval(args.rest.first)
    if args.rest.rest is nil:
        return result
    return floor_div(Pair(result, args.rest.rest))
